...
Grid.prototype.getFirstWithTwoInARow = function (agent) {
    if (agent !== computer && agent !== player && agent !== playerTwo) {
...
function cellClicked(id) {
    ...
    if (myGrid.cells[cell] > 0 || whoseTurn !== player && whoseTurn !== playerTwo || gameOver) {
    ...
    if (whoseTurn === player) {
        ...
        document.getElementById(id).innerHTML = playerTwoText;
    }
...
function checkWin() {
    ...
    for (var i = 0; i <= 2; i++) {
        ...
        if (row[0] > 0 && row[0] === row[1] && row[0] === row[2]) {
            if (who === 1 && row[0] === computer) {
                score.computer++;
                winner = computer;
            } else if (who === 0 && row[0] === playerTwo) {
                score.playerTwo++;
                winner = playerTwo;
            }else if (who === 0 && row[0] === player || who === 1 && row[0] === player) {
                score.player++;
                winner = player;
            }
    ...
    for (i = 0; i <= 2; i++) {
        ...
        if (col[0] > 0 && col[0] === col[1] && col[0] === col[2]) {
            if (who === 1 && col[0] === computer) {
                score.computer++;
                winner = computer;
            } else if (who === 0 && col[0] === playerTwo) {
                score.playerTwo++;
                winner = playerTwo;
            } else if (who === 0 && col[0] === player || who === 1 && col[0] === player) {
                score.player++;
                winner = player;
            }
            // Give the winning row/column/diagonal a different bg-color
    ...
    for (i = 0; i <= 1; i++) {
        ...
        if (diagonal[0] > 0 && diagonal[0] === diagonal[1] && diagonal[0] === diagonal[2]) {
            if (who === 1 && diagonal[0] === computer) {
                score.computer++;
                winner = computer;
            } else if (who === 0 && diagonal[0] === playerTwo) {
                score.playerTwo++;
                winner = playerTwo;
            } else if (who === 0 && diagonal[0] === player || who === 1 && diagonal[0] === player) {
                score.player++;
                winner = player;
            }
            // Give the winning row/column/diagonal a different bg-color
...
function announceWinner(text) {
    ...
    setTimeout(closeModal, 3400, "winAnnounce");
}
...
function showOptions() {
    ...
    if (who === 0) {
        ...
        document.getElementById('twoplay').checked = true;
    }
    else {
        document.getElementById('comp').checked = true;
        document.getElementById('twoplay').checked = false;
    }
...
function getOptions() {
    ...
    if (document.getElementById('twoplay').checked === true) {
        who = 0;
        player = x;
        playerTwo = o;
        whoseTurn = player;
...
function endGame(who) {
    ...
    document.getElementById("playerTwo_score").innerHTML = score.playerTwo;
    for (var i = 0; i <= 8; i++) {
        var id = "cell" + i.toString();
        document.getElementById(id).style.cursor = "default";
    }
